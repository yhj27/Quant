...
from config import (
    ...
    LADDER_RSI_LEVELS,
    RSI_TP1, RSI_TP2,           # ★ 추가
    PARTIAL_SELL_RATIO,         # ★ 추가
    EMA_EXIT_PERIOD, USE_EMA_EXIT,  # ★ 추가
    LADDER_USE_PRICE_FILTER, LADDER_PRICE_STEP_PCT,  # ★ 추가
...
def apply_15m_indicators(df):
    ...
    # 15분봉 EMA (강제 청산용) - 기간은 config에서
    df["ema_exit"] = df["close"].ewm(span=EMA_EXIT_PERIOD, adjust=False).mean()
...
def backtest_rsi_portfolio_ladder(df, buy_rsi, sell_rsi):
    """
    매수:
      - 1h EMA200 상방(trend_ok=True)
      - 15m RSI <= LADDER_RSI_LEVELS[0]
      - 15m close <= BB_LOW
      - Ladder: LADDER_RSI_LEVELS에 따라 추가 분할매수

    매도:
      - 보조 강제청산: USE_EMA_EXIT=True 이고 15m close < ema_exit  → 전량 청산
      - 1차 익절: RSI >= RSI_TP1 & 아직 partial 안 했을 때 → PARTIAL_SELL_RATIO 만큼 청산
      - 2차 익절: RSI >= RSI_TP2 → 남은 물량 전량 청산

    ※ buy_rsi, sell_rsi 인자는 시그니처 유지용으로만 남겨둠 (실제 로직은 config 값 사용)
    """
    ...
    for i in range(len(df) - 1):
        ...
        #bb_mid      = df.loc[i, "bb_mid"]
        ema_exit    = df.loc[i, "ema_exit"]
        ...
        if first_buy_cond or add_ladder_cond:
            ...
            if POSITION_MODE == "fixed_quote":
                ...
                # 라더 단계에 따라 weight 선택
                # 1차 매수 → weight[0], 2차 → weight[1], 3차 → weight[2] ...
                if first_buy_cond:
                    w_idx = 0
                else:
                    w_idx = ladder_level  # 현재 ladder_level이 1이면 두 번째 라더

                # 인덱스가 리스트 길이를 넘으면 마지막 weight를 사용
        ...
        if position > 0:
            # 0) 보조 강제 청산: EMA 이탈
            if USE_EMA_EXIT and close_price < ema_exit:
            ...
            # 1) 1차 익절: RSI >= RSI_TP1 & 아직 partial X
            if (not partial_taken) and rsi_val >= RSI_TP1:
                ...
                partial_taken = True
                # 나머지는 계속 홀딩 → 다음 캔들에서 2차 익절/EMA 체크
                continue
...
def perf_summary_portfolio(trade_rets, df_equity, trades):
    ...
    print(f"트레이드 횟수: {len(trade_rets)}")
